...
do_run_unsup_deconvolution = function(method, dat, option=c("Tmat","Amat"), prop_simu=NULL, ref_profiles=NULL, dist=F, threads=32) {
  ...
  if (method=="MeDeCom") {
    ...
    T_ <- MeDeCom::getLMCs(res, K=k, lambda=lambda)
    res <- list(A_matrix=A,
                T_matrix=T_)
  }
  else if (method=="ICA") {
...
SB_deconv_lot_method_sim(lot, block, method, method_class, ifelse(sim=="10",10,as.numeric(strsplit(sim,"")[[1]][2])), date, input_path, pred_file, time_file fs)
